...
static void     config_threads(void)
{
    ...
    // TODO: (Vizonex) When or if I megere to duplicut give seperate error reasonings.
    // Such as renaming sysconf(...) to sys_info->... 
    if (max_threads < 0)
...
void            config(void)
{
    ...
    // printf("init_memstate();");
    init_memstate(&memstate);

    // printf("config_threads();");
    config_threads();
    // printf("config_hmap_size()");
    config_hmap_size(g_file, &memstate);
    // printf("config_chunk_size();");
